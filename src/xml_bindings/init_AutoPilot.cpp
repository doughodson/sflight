...
void init_AutoPilot(xml::Node* node)
{
   ...
   for (int i = 0; i < comps.size(); i++) {
      ...
      if (xml::get(tmp, "Type", "") == "HeadingHold") {
         ...
         autoPilot->maxBankRate = mdls::UnitConvert::toRads(xml::getDouble(tmp, "MaxBankRate", autoPilot->maxBankRate));
         ...
         autoPilot->maxG = (xml::getDouble(tmp, "MaxG", autoPilot->maxG) - 1) * mdls::nav::getG(0, 0, 0);
         autoPilot->minG = (xml::getDouble(tmp, "MinG", autoPilot->minG) - 1) * mdls::nav::getG(0, 0, 0);
         autoPilot->maxG_rate = autoPilot->maxG;
         autoPilot->minG_rate = autoPilot->minG;
         player->autoPilotCmds.setMaxPitchUp(
             mdls::UnitConvert::toRads(xml::getDouble(tmp, "MaxPitchUp", mdls::math::PI / 2.0)));
         player->autoPilotCmds.setMaxPitchDown(
             mdls::UnitConvert::toRads(xml::getDouble(tmp, "MaxPitchDown", -mdls::math::PI / 2.0)));
