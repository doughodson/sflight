...
namespace sf {
    ...
    void Table2D :: setData(string data) {
        
        vector <string> v = NodeUtil::splitString(data, ',');
        
        if( v.size() != numRows * numCols ) return;
    ...
    double** Table2D :: getData() {
        return this->data;
    }
    
    double* Table2D  :: getColVals() {
        return colVals;
    }
    
    double* Table2D  :: getRowVals() {
        return rowVals;
    }
    ...
    void Table2D :: set(int row, int col, double val) {
        data[row][col] = val;
    }
    
    double Table2D :: get(int row, int col) {
        return data[row][col];
    }
